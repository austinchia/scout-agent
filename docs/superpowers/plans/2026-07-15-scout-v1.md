...
**Tech Stack:** Python 3.11+, uv, FastAPI, Pydantic v2, psycopg 3, Google Gen AI Python SDK (`google-genai`: gemini-flash-lite-latest, gemini-embedding-001), Tavily Python SDK (`tavily-python`), Neon Postgres + pgvector, React + Vite + Tailwind v4, pnpm (via Corepack).
...
- Model usage: `gemini-flash-lite-latest` for classification/synthesis/talking points/sufficiency check (originally `gemini-2.5-flash`/`gemini-2.5-flash-lite`, updated 2026-07-16 — see Task 8's live-verification amendment below), `gemini-embedding-001` (with `output_dimensionality=1536`) for embeddings — the 1536 dimension is required so embeddings stay compatible with the `VECTOR(1536)` columns Task 4 already migrated.
...
**Amendment (2026-07-16, discovered during Task 8's live verification):** `gemini-2.5-flash` and `gemini-2.5-flash-lite` (used above) turned out to be no longer available to new API keys — Google had moved on to newer model generations by the time this was tested live. `gemini-flash-latest` (the current full-tier model) also returned repeated `503 UNAVAILABLE` ("experiencing high demand") when checked directly against the real API key. `gemini-flash-lite-latest` was confirmed reliably available and was used to replace both `FLASH_MODEL` and `FLASH_LITE_MODEL` in `gemini_client.py`, with the corresponding test assertions updated to match. If you're implementing this task fresh, verify current model availability against your own API key before trusting the model names in the code block above — model naming and availability shift over time in ways a static plan can't track.
...
**Package manager:** `pnpm`, not `npm` — enabled via Node's built-in Corepack rather than a separate global install.
...
```
Expected: `pnpm --version` now resolves (Corepack fetches pnpm on first use).
...
- [ ] **Step 4: Wire the Tailwind Vite plugin and dev proxy**
...
- [ ] **Step 5: Import Tailwind in the stylesheet**
...
- [ ] **Step 8: Add a root .gitignore entry for frontend build artifacts**
...
```bash
git add frontend
git commit -m "Add minimal React + Vite + Tailwind frontend for Scout, using pnpm"
```
