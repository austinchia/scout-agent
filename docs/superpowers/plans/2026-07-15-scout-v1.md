...
**Architecture:** FastAPI backend (`backend/app`) with a hand-rolled agent loop (no framework) calling the OpenAI API for search sufficiency checks, structured classification, embeddings, and drafting. Neon Postgres (`pgvector`) stores `profiles`, `reference_docs`, `scout_runs`, and `service_lines`. A minimal React + Vite + Tailwind frontend (`frontend`) posts to `POST /scout/run` and renders the result.

**Tech Stack:** Python 3.11+, uv, FastAPI, Pydantic v2, psycopg 3, OpenAI Python SDK (gpt-4o, gpt-4o-mini, text-embedding-3-small), Neon Postgres + pgvector, React + Vite + Tailwind v4.

## Global Constraints

- v1 scope only: no deck generation, no Notion write-back (both deferred per PRD section 11).
- Model usage per PRD section 8: `gpt-4o` for classification/synthesis/talking points, `gpt-4o-mini` for the sufficiency check, `text-embedding-3-small` for embeddings.
...
def _fake_response():
    ...
    content = SimpleNamespace(type="output_text", text="Clarins is a cosmetics company.", annotations=[annotation])
    message = SimpleNamespace(type="message", content=[content])
    return SimpleNamespace(output=[message])


def test_extract_results_parses_url_citations():
    results = _extract_results(_fake_response())
    assert results == [SearchResult(title="Clarins overview", url="https://example.com/clarins", snippet="Clarins")]


def test_extract_results_deduplicates_urls():
...
def test_extract_results_returns_empty_list_for_no_citations():
...
def _extract_results(response) -> list[SearchResult]:
    ...
    for item in getattr(response, "output", []):
        ...
        for content in getattr(item, "content", []):
            for annotation in getattr(content, "annotations", None) or []:
                ...
                snippet = content.text[start:end] if hasattr(content, "text") else ""
                results.append(
                    SearchResult(title=getattr(annotation, "title", url), url=url, snippet=snippet)
                )
    return results
...
```bash
git add backend/app/agent/search.py backend/tests/test_search.py
git commit -m "Add web search tool using OpenAI Responses API"
```

### Task 6: OpenAI client — sufficiency, classification, synthesis, talking points
...
**Interfaces:**
- Consumes: `SearchResult` from Task 5, `Classification`/`ServiceLine` from Task 2, `OPENAI_API_KEY` env var.
- Produces (used by Task 7's `loop.py`):
...
All five accept an optional `client` parameter for dependency injection in tests.
...
def test_embed_text_returns_embedding_vector():
    ...
    fake_client.embeddings.create.assert_called_once_with(model="text-embedding-3-small", input="Groupe Clarins")


def test_evaluate_sufficiency_true_on_yes():
    fake_client = MagicMock()
    fake_client.chat.completions.create.return_value = MagicMock(
        choices=[MagicMock(message=MagicMock(content="Yes, enough detail."))]
    )
...
def test_evaluate_sufficiency_false_on_no():
    fake_client = MagicMock()
    fake_client.chat.completions.create.return_value = MagicMock(choices=[MagicMock(message=MagicMock(content="No"))])
...
def test_classify_returns_parsed_classification():
    ...
    fake_client.chat.completions.parse.return_value = MagicMock(
        choices=[MagicMock(message=MagicMock(parsed=expected))]
    )
    ...
    assert kwargs["response_format"] is Classification


def test_synthesize_brief_splits_brief_and_rationale():
    fake_client = MagicMock()
    fake_client.chat.completions.create.return_value = MagicMock(
        choices=[MagicMock(message=MagicMock(content="Company overview here.\n---\nWhy this fits."))]
    )
...
def test_generate_talking_points_parses_lines():
    fake_client = MagicMock()
    fake_client.chat.completions.create.return_value = MagicMock(
        choices=[MagicMock(message=MagicMock(content="- What tools do you use?\n- Who owns reporting?\n"))]
    )
    classification = Classification(service_line="training", confidence=0.9, rationale="x")
...
Expected: `FAIL` with `ModuleNotFoundError: No module named 'app.agent.openai_client'`

- [ ] **Step 3: Write the openai_client module**
...
def evaluate_sufficiency(search_results: list[list[SearchResult]], client: OpenAI | None = None) -> bool:
    client = client or get_client()
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": _flatten(search_results) or "(no results yet)"},
        ],
    )
...
def classify(
    ...
    completion = client.chat.completions.parse(
        ...
        messages=[
            ...
            {"role": "user", "content": _flatten(search_results)},
        ],
...
def generate_talking_points(
    ...
    lines = [line.strip("-* ").strip() for line in completion.choices[0].message.content.splitlines()]
...
```bash
git add backend/app/agent/openai_client.py backend/tests/test_openai_client.py
git commit -m "Add OpenAI client for sufficiency, classification, synthesis, and talking points"
```
...
**Interfaces:**
- Consumes: `search.web_search` (Task 5); `openai_client.{evaluate_sufficiency, classify, embed_text, synthesize_brief, generate_talking_points}` (Task 6); `queries.{get_active_service_lines, vector_search, upsert_profile, insert_scout_run}` and `get_connection` (Task 3/4); `ScoutProfile`, `ServiceLine` (Task 2).
...
def _patch_common(monkeypatch, classification, talking_points=("Question one?", "Question two?")):
    ...
    monkeypatch.setattr(loop.openai_client, "classify", lambda results, lines: classification)
    monkeypatch.setattr(loop.openai_client, "embed_text", lambda text: [0.1, 0.2])
    monkeypatch.setattr(loop.queries, "vector_search", lambda conn, table, embedding, top_k=5: [])
    monkeypatch.setattr(
        loop.openai_client, "synthesize_brief", lambda results, classification, grounding: ("Brief text", "Rationale text")
    )
    monkeypatch.setattr(
        loop.openai_client, "generate_talking_points", lambda brief, classification, grounding: list(talking_points)
    )
...
def test_run_scout_orchestrates_full_pipeline(monkeypatch):
    ...
    monkeypatch.setattr(loop.openai_client, "evaluate_sufficiency", lambda results: True)
    monkeypatch.setattr(
...
def test_run_scout_flags_low_confidence(monkeypatch):
    ...
    monkeypatch.setattr(loop.openai_client, "evaluate_sufficiency", lambda results: False)
...
def test_run_scout_survives_a_failed_search_attempt(monkeypatch):
    ...
    monkeypatch.setattr(loop.openai_client, "evaluate_sufficiency", lambda results: bool(results[-1]))
...
from app.agent import openai_client, search
from app.db import queries
from app.db.connection import get_connection
from app.models.profile import ScoutProfile
from app.models.service_line import ServiceLine
...
def run_scout(company_name: str, note: str | None = None) -> ScoutProfile:
    ...
    while not sufficient and attempts < MAX_SEARCH_ATTEMPTS:
        ...
        sufficient = openai_client.evaluate_sufficiency(search_results)
        attempts += 1

    classification = openai_client.classify(search_results, service_lines)

    query_embedding = openai_client.embed_text(f"{company_name} {classification.service_line}")
    with get_connection() as conn:
    ...
    brief, rationale = openai_client.synthesize_brief(search_results, classification, reference_chunks)
    talking_points = openai_client.generate_talking_points(brief, classification, reference_chunks)
    ...
    profile_embedding = openai_client.embed_text(f"{company_name} {note or ''} {brief}")
    with get_connection() as conn:
        profile_id = queries.upsert_profile(
            conn,
            {
...
```
Expected: a JSON `ScoutProfile` response within ~2 minutes (this hits real OpenAI + Neon, so it costs a small amount of API usage — expected per the NFR in PRD section 9).
